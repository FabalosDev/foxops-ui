<script lang="ts">
  ...
  // Accept the calculated metrics
  export let metrics = { totalOps: 0, activeTickets: 0 };
        ...
        <div class="space-y-2">
          ...
          <div class="flex items-center justify-between p-2 rounded bg-white/5 border border-white/5">
            <span class="flex items-center gap-2 text-white/60">
              <Activity size={12}/> Total Ops
            </span>
            <span class="text-white font-bold">{fmt(metrics.totalOps)}</span>
          </div>
          ...
          <div class="flex items-center justify-between p-2 rounded bg-white/5 border border-white/5">
            ...
            <span class="{metrics.activeTickets > 0 ? 'text-orange-500' : 'text-emerald-500'} font-bold">
              {metrics.activeTickets > 0 ? 'HEALING...' : 'IDLE'}
            </span>
          </div>
        </div>
      </div>
    </div>
    <div class="border-t border-white/5 pt-8 flex flex-col md:flex-row justify-between items-center gap-6 opacity-60">
